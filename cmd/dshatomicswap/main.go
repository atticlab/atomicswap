...
import (
	...
	"github.com/xbis/godash/chaincfg"
	"github.com/xbis/godash/chaincfg/chainhash"
	rpc "github.com/xbis/godash/rpcclient"
	//"github.com/xbis/godash/txrules"
...
var (
	...
	connectFlag = flagset.String("s", "127.0.0.1", "host[:port] of Bitcoin Core wallet RPC server")
...
func init() {
	flagset.Usage = func() {
		fmt.Println("Usage: dshatomicswap [flags] cmd [cmd args]")
		fmt.Println()
...
type initiateCmd struct {
	cp2Addr *godashutil.AddressPubKeyHash
	amount  godashutil.Amount
}

type participateCmd struct {
	cp1Addr    *godashutil.AddressPubKeyHash
	amount     godashutil.Amount
	secretHash []byte
}
...
func run() (err error, showUsage bool) {
	...
	switch args[0] {
	case "initiate":
		cp2Addr, err := godashutil.DecodeAddress(args[1], chainParams)
		if err != nil {
		...
		cp2AddrP2PKH, ok := cp2Addr.(*godashutil.AddressPubKeyHash)
		if !ok {
		...
		amount, err := godashutil.NewAmount(amountF64)
		if err != nil {
			return err, true
		}
	...
	case "participate":
		cp1Addr, err := godashutil.DecodeAddress(args[1], chainParams)
		if err != nil {
		...
		cp1AddrP2PKH, ok := cp1Addr.(*godashutil.AddressPubKeyHash)
		if !ok {
...
func createSig(tx *wire.MsgTx, idx int, pkScript []byte, addr godashutil.Address,
...
func fundRawTransaction(c *rpc.Client, tx *wire.MsgTx, feePerKb godashutil.Amount) (fundedTx *wire.MsgTx, fee godashutil.Amount, err error) {
	...
	feeAmount, err := godashutil.NewAmount(resp.Fee)
	if err != nil {
		return nil, 0, err
	}
...
func getFeePerKb(c *rpc.Client) (useFee, relayFee godashutil.Amount, err error) {
	info, err := c.GetInfo()
	if err != nil {
		return 0, 0, fmt.Errorf("getinfo: %v", err)
	}
	relayFee, err = godashutil.NewAmount(info.RelayFee)
	if err != nil {
		return 0, 0, err
	}
	...
	if info.PaytxFee != 0 {
		...
		useFee, err = godashutil.NewAmount(maxFee)
	...
	if err == nil && estimateResp != -1 {
		useFee, err = godashutil.NewAmount(estimateResp)
		if relayFee > useFee {
			useFee = relayFee
		}
		return useFee, relayFee, err
	...
	useFee, err = godashutil.NewAmount(info.RelayFee)
	return useFee, relayFee, err
}
...
func getRawChangeAddress(c *rpc.Client) (godashutil.Address, error) {
	...
	addr, err := godashutil.DecodeAddress(addrStr, chainParams)
	if err != nil {
		return nil, err
	}
...
type contractArgs struct {
	them       *godashutil.AddressPubKeyHash
	amount     godashutil.Amount
	locktime   int64
	secretHash []byte
}
...
type builtContract struct {
	contract       []byte
	contractP2SH   godashutil.Address
	contractTxHash *chainhash.Hash
	contractTx     *wire.MsgTx
	contractFee    godashutil.Amount
	refundTx       *wire.MsgTx
	refundFee      godashutil.Amount
}
...
func buildContract(c *rpc.Client, args *contractArgs) (*builtContract, error) {
	...
	contractP2SH, err := godashutil.NewAddressScriptHash(contract, chainParams)
	if err != nil {
		return nil, err
	}
...
func buildRefund(c *rpc.Client, contract []byte, contractTx *wire.MsgTx, feePerKb, minFeePerKb godashutil.Amount) (
	refundTx *wire.MsgTx, refundFee godashutil.Amount, err error) {
	...
	refundAddr, err := godashutil.NewAddressPubKeyHash(pushes.RefundHash160[:], chainParams)
	if err != nil {
		return nil, 0, err
	}
	...
	refundFee = FeeForSerializeSize(feePerKb, refundSize)
	refundTx.TxOut[0].Value = contractTx.TxOut[contractOutPoint.Index].Value - int64(refundFee)
	if IsDustOutput(refundTx.TxOut[0], minFeePerKb) {
		return nil, 0, fmt.Errorf("refund output value of %v is dust", godashutil.Amount(refundTx.TxOut[0].Value))
	}
...
func calcFeePerKb(absoluteFee godashutil.Amount, serializeSize int) float64 {
...
func (cmd *initiateCmd) runCommand(c *rpc.Client) error {
	...
	//locktime := time.Now().Add(2 * time.Minute).Unix()
	...
	fmt.Printf("Contract fee: %v (%0.8f DASH /kB)\n", b.contractFee, contractFeePerKb)
	fmt.Printf("Refund fee:   %v (%0.8f DASH /kB)\n\n", b.refundFee, refundFeePerKb)
...
func (cmd *redeemCmd) runCommand(c *rpc.Client) error {
	...
	recipientAddr, err := godashutil.NewAddressPubKeyHash(pushes.RecipientHash160[:],
		chainParams)
	if err != nil {
		return err
	}
	contractHash := godashutil.Hash160(cmd.contract)
	contractOut := -1
	for i, out := range cmd.contractTx.TxOut {
		...
		if sc == txscript.ScriptHashTy &&
			bytes.Equal(addrs[0].(*godashutil.AddressScriptHash).Hash160()[:], contractHash) {
			contractOut = i
			break
		}
	}
	...
	fee := FeeForSerializeSize(feePerKb, redeemSize)
	redeemTx.TxOut[0].Value = cmd.contractTx.TxOut[contractOut].Value - int64(fee)
	if IsDustOutput(redeemTx.TxOut[0], minFeePerKb) {
		return fmt.Errorf("redeem output value of %v is dust", godashutil.Amount(redeemTx.TxOut[0].Value))
	}
	...
	fmt.Printf("Redeem fee: %v (%0.8f DASH/kB)\n\n", fee, redeemFeePerKb)
...
func (cmd *auditContractCmd) runOfflineCommand() error {
	contractHash160 := godashutil.Hash160(cmd.contract)
	contractOut := -1
	for i, out := range cmd.contractTx.TxOut {
		...
		if bytes.Equal(addrs[0].(*godashutil.AddressScriptHash).Hash160()[:], contractHash160) {
			contractOut = i
			break
		}
	}
	...
	contractAddr, err := godashutil.NewAddressScriptHash(cmd.contract, chainParams)
	if err != nil {
		return err
	}
	recipientAddr, err := godashutil.NewAddressPubKeyHash(pushes.RecipientHash160[:],
	...
	refundAddr, err := godashutil.NewAddressPubKeyHash(pushes.RefundHash160[:],
		chainParams)
	if err != nil {
		return err
	}
	...
	fmt.Printf("Contract value:          %v\n", godashutil.Amount(cmd.contractTx.TxOut[contractOut].Value))
